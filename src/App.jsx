...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError('网络错误，请检查是否已部署到 Cloudflare Pages')
    } finally {
          ...
          {view === 'login' ? (
            <>
              ...
              <form onSubmit={handleLogin}>
                <div className="field">
                  <label>邮箱</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required disabled={submitLoading} />
                </div>
                <div className="field">
                  <label>密码</label>
                ...
                {error && <p className="api-msg error">{error}</p>}
                {success && <p className="api-msg success">{success}</p>}
              ...
              <p className="foot">还没有账号？<a href="#" onClick={(e) => { e.preventDefault(); handleViewSwitch('register'); }}>立即注册</a></p>
            </>
          ) : (
            <>
              <h2>创建账号</h2>
              ...
              <p className="foot">已有账号？<a href="#" onClick={(e) => { e.preventDefault(); handleViewSwitch('login'); }}>立即登录</a></p>
            </>
          )}
        </div>
      </div>
